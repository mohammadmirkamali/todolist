export const GET_CHAPTER_REQUEST = 'GET_CHAPTER_REQUEST';
export const GET_CHAPTER_SUCCESS = 'GET_CHAPTER_SUCCESS';
export const GET_CHAPTER_ERROR = 'GET_CHAPTER_ERROR';

export const GET_SEARCH_DATA_REQUEST = 'GET_SEARCH_DATA_REQUEST';
export const GET_SEARCH_DATA_SUCCESS = 'GET_SEARCH_DATA_SUCCESS';
export const GET_SEARCH_DATA_ERROR = 'GET_SEARCH_DATA_ERROR';

export const GET_POSTS_REQUEST = 'GET_POSTS_REQUEST';
export const GET_POSTS_SUCCESS = 'GET_POSTS_SUCCESS';
export const GET_POSTS_ERROR = 'GET_POSTS_ERROR';

export const GET_EXAM_INFO_REQUEST = 'GET_EXAM_INFO_REQUEST';
export const GET_EXAM_INFO_SUCCESS = 'GET_EXAM_INFO_SUCCESS';
export const GET_EXAM_INFO_ERROR = 'GET_EXAM_INFO_ERROR';

export const GET_ALL_WEBINAR_REQUEST = 'GET_EVENT_REQUEST';
export const GET_ALL_WEBINAR_SUCCESS = 'GET_EVENT_SUCCESS';
export const GET_ALL_WEBINAR_ERROR = 'GET_EVENT_ERROR';

export const GET_EVENT_REQUEST = 'GET_EVENT_REQUEST';
export const GET_EVENT_SUCCESS = 'GET_EVENT_SUCCESS';
export const GET_EVENT_ERROR = 'GET_EVENT_ERROR';

export const GET_TERM_REQUEST = 'GET_TERM_REQUEST';
export const GET_TERM_SUCCESS = 'GET_TERM_SUCCESS';
export const GET_TERM_ERROR = 'GET_TERM_ERROR';

export const GET_HOME_REQUEST = 'GET_HOME_REQUEST';
export const GET_HOME_SUCCESS = 'GET_HOME_SUCCESS';
export const GET_HOME_ERROR = 'GET_HOME_ERROR';

export const CHANGE_TERM_HOUR_REQUEST = 'CHANGE_TERM_HOUR_REQUEST';
export const CHANGE_TERM_HOUR_SUCCESS = 'CHANGE_TERM_HOUR_SUCCESS';
export const CHANGE_TERM_HOUR_ERROR = 'CHANGE_TERM_HOUR_ERROR';

export const UPDATE_COURSE = 'UPDATE_COURSE';
export const CLEAR_STORE = 'CLEAR_STORE';
